...
If a task requires Amazon login, first check whether the user supplied a pre-authorized login and verification workflow. Use that workflow only for the user's own account and never print credentials, cookies, OTP seeds, phone numbers, or one-time codes. If the browser is not logged into an Amazon buyer account and no usable login workflow is available, stop all capture actions and ask for the Amazon buyer login method through the agent's normal user-facing reply or approved secret channel, not through terminal stdin or blocking `input()` prompts. If Amazon forces an "Add a mobile number" or mobile-number verification page, stop Rufus and Listing capture, ask the human for the phone number through an approved channel, submit it, report `已添加手机号`, then ask for the SMS verification code and enter it before resuming. If a task requires typing persona details into Amazon profile features such as "Tell us about you", stop and ask for explicit confirmation through the chat/user interface before entering that data. Prefer labeling personas in the audit table without changing the user's Amazon account profile.
...
1. Establish the audit set.
   ...
   - If Amazon shows an "Add a mobile number" or mobile phone verification screen, mark `challenge_type=mobile_number_required`, pause all other actions, ask the human for the phone number, then ask for the received SMS code after the number is submitted.
